...
export default {
  ...
  methods: {
    getsignup() {
    axios.post("http://localhost:5000/user/signup",this.signupForm)
      .then((response) => {
      if(response.data=="username already taken"){
        alert('username already taken try a new one');
        this.$router.push('/signup_');
      }
      if(response.data=="email already exist"){
        alert("email already exsit add a new one");
      }
      else{
     ...
     .catch((error) => {
        console.log('erreur is ',error,this.signupForm);
    });
    },
  },
};
